...
const String _modelPath = 'assets/models/ssd_mobilenet.tflite';
const String _labelPath = 'assets/models/labelmap.txt';
                        ...
                        decoration: BoxDecoration(
                          ...
                          boxShadow: [
                            BoxShadow(
                              offset: Offset(0, 10),
                              blurRadius: 33,
                              color: Color(0xFFD3D3D3).withOpacity(0.01),
                            ),
                          ],
                        ),
                      ),
    ...
    try {
      ...
      await cameraController.startImageStream((CameraImage image) {
        ...
        if (!isDetecting) {
          ...
          final imageData = image.planes.map((plane) {
            return plane.bytes;
          }).toList();

          objectDetection?.analyseImage2(imageData[0]);
      ...
      await cameraController.startImageStream((CameraImage cameraImage) async {
        ...
        img.Image test = img.Image.fromBytes(
            width: width, height: height, bytes: list.buffer);

        List<int> _imageBytes = img.encodeJpg(test);
